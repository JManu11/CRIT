...
class LoginApp:
    def __init__(self, root):
        ...
        self.label = ctk.CTkLabel(self.frame, text="Por favor, ingrese su nombre y contraseña:", font=('Arial', 16))
        self.label.pack(pady=10)

        self.name_entry = ctk.CTkEntry(self.frame, width=220, font=('Microsoft YaHei UI Light', 11))
        self.name_entry.pack(pady=5)
        self.name_entry.insert(0, 'Nombre de usuario')

        self.password_entry = ctk.CTkEntry(self.frame, width=220, font=('Microsoft YaHei UI Light', 11), show='*')
        self.password_entry.pack(pady=5)
        self.password_entry.insert(0, 'Contraseña')

        self.signin_button = ctk.CTkButton(self.frame, text="Iniciar Sesión", command=self.signin)
        self.signin_button.pack(pady=10)

        sign_up = ctk.CTkButton(self.frame, width=100, height=40, text='Registrar', fg_color='#57a1f8', corner_radius=10, command=open_register_window)
        sign_up.place(x=125, y=280)

        self.capture_button = ctk.CTkButton(self.frame, text="Capturar", command=self.open_camera)
        self.capture_button.pack(pady=10)

        self.video_frame = ctk.CTkLabel(self.frame)
        self.video_frame.pack()

        self.login_button = ctk.CTkButton(self.frame, text="Verificar Rostro", command=self.prompt_login, state=ctk.DISABLED)
    ...
    def prompt_login(self):
        self.label.config(text="Por favor, verifique su rostro para iniciar sesión:")
