...
def main():
    ...
    global data  # Asegurar que data es global para poder actualizarla

    with conexion.cursor() as cursor:
       cursor.execute("select *from info_detallada;")

    filas = cursor.fetchall()

    # Estructurar los datos como en el ejemplo inicial
    encabezados = ['N.Expediente', 'Nombre', 'Apellido Paterno' ,'Fecha Nacimiento' , 'Núm. Requisición', 'Artículo Solicitado', 'Costo Aproximado',
                   'Estado de Pago', 'Num. Factura', 'Proveedor', 'Folio de Pago \n' ]
    data = [encabezados] + [list(fila) for fila in filas]
    ...
    def  agregar_paciente(): 
        def autocompletar_datos():
            try:
                ...
                # Llamar al procedimiento almacenado para obtener los datos del paciente
                cursor.callproc("obtener_datos_usuario", (id, v_nombre_usuario, v_apellido_usuario, v_fecha_nacimiento))
                
                # Recuperar los valores de las variables de salida del procedimiento almacenado
                cursor.execute("SELECT @nombre_usuario, @apellido_usuario, @edad_usuario")
                result = cursor.fetchone()
                
                # Asignar los valores recuperados a las variables locales
                nombre = result[0]
                apellido = result[1]
                edad = result[2]
                
                # Asignar los valores recuperados a los campos de entrada correspondientes en la interfaz de usuario
        ...
        data.append([num_req, entry_nombre, entry_apellido, entry_edad, num_req, articulo, costo, estado_pago,num_fac,proveedor,fol_Pago])
   ...
   # button_autocompletar = Button(root, text="Autocompletar", command=autocompletar_datos)
   # button_autocompletar.grid(row=5, column=8, columnspan=5)
